...
LET_ASSERT_RESULT LET_assert_printer_expected_result;
char *LET_assert_printer_expected_file;
uint32_t LET_assert_printer_expected_line;
...
void Reset_env(void){
  ...
  LET_assert_printer_expected_file = null_char;
  ...
  LET_assert_printer_expected_line = 0;
  ...
  LET_assert_printer_expected_result = 0;
...
void LET_assert_printer(char *name,
  ...
  if(LET_assert_printer_expected_result == result){
    printf("OK\n");
  }else {
    printf("LET_assert_printer result  KO\n");
  }
  if(!strcmp(LET_assert_printer_expected_file, file)){
    printf("OK\n");
  }else {
    printf("LET_assert_printer file  KO\n");
  }
  if(LET_assert_printer_expected_line == line){
    printf("OK\n");
  }else {
    printf("LET_assert_printer line  KO\n");
  }
  count_LET_assert_printer++;
}

void LET_uint_to_decimal_string(char str[], uint64_t num){
  if(!strcmp(LET_uint_to_decimal_string_expected_str[count_LET_uint_to_base_string], str)){
    printf("OK\n");
  }else {
    printf("LET_uint_to_decimal_string str  KO\n");
  }
  if(LET_uint_to_decimal_string_expected_num[count_LET_uint_to_base_string] == num){
    printf("OK\n");
  }else {
...
uint32_t LET_str_size(char *a){
  if(!strcmp(LET_str_size_expected_a[count_LET_compare_str], a)){
    printf("OK\n");
  }else {
