...
LET_ASSERT_RESULT LET_test_end_printer_expected_result[LET_MAX_TESTS];
...
void Reset_env(void){
  ...
  for(uint8_t i = 0; i<LET_MAX_TESTS; i++){
    ...
    LET_test_end_printer_expected_result[i] = 0;
...
void LET_test_end_printer(LET_ASSERT_RESULT result){
  if(LET_test_end_printer_expected_result[count_LET_test_end_printer] == result){
    printf("OK\n");
  }else {
    printf("LET_test_end_printer result  KO\n");
  }
...
void test_LET_service_runner(void){
    ...
    LET_test_end_printer_expected_result[0] = LET_OK;
    LET_test_end_printer_expected_result[1] = LET_KO;
    LET_test_register(&service_test, test_2_name, stub_call_b);
    service_test.test_list[1].result = LET_KO;
